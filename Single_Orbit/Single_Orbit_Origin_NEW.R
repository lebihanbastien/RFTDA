...
# This file plots on a single figure the dyneq computed in NCSYS (NCEM or NCSEM)
# This file makes use of the date files of type (example) "DYNEQ_QBCP_EM_L1_NC.txt" 
# contains in the folder "OOFTDA/plot/QBCP/DYNEQ/..."
...
orbit = read.table(paste0(currentfolder,name,".txt"), header = F)
colnames(orbit) = c("xEM", "yEM")
orbit$type = name
...
# SYS to NC
#------------------------------------------------
orbit$xNC <- -1/LIB$GAMMA*orbit$xEM + LIB$C1 #-gamma * (df$x-c1);
orbit$yNC <- -1/LIB$GAMMA*orbit$yEM #-gamma * (df$y);


#------------------------------------------------
# SYS to PH
#------------------------------------------------
orbit$xPH = orbit$xEM * LIB$LPRIM
orbit$yPH = orbit$yEM * LIB$LPRIM
...
#Path
#------------------------------------------------
porb = fplot_path (orbit,  xCOL, yCOL,  xLAB,  yLAB, "type", "", TRUE, "type")

#------------------------------------------------
# Starting point
#------------------------------------------------
porb = porb + geom_point(data = orbit_start, aes(xEM, yEM), size = 4, colour = "black", fill = "black", pch = 21)
...
ai= 100
porb = porb + geom_segment(aes(x = orbit$xEM[ai], y = orbit$yEM[ai], 
                               xend = orbit$xEM[ai+1], yend = orbit$yEM[ai+1]), 
                           colour = "black", 
...
if(Li == "L1" && FWRK == "EM"){
  porb = porb + annotate("text", x = Ldf$xEM+1e-07, y = Ldf$yEM+5e-07, label = "$L_1$", size = 10)#, parse =TRUE)
}
