...
size =  "40_test" #40_test
sizep = "40_test" #40_test
...
pI = pI + scale_y_log10(breaks = 10^(-8:0*2), limits=c(1e-10,1e-2))
#Theme
...
pxyPH = pxyPH + geom_path(data  = orbitMaxOrder, aes(xPH, yPH, color = "PM"), size = linesize[["line"]], linetype = "dotted", colour = "black")

# Starting point
pxyPH = pxyPH + geom_point(data = orbitMaxOrder_start, aes(xPH, yPH), size = 4, colour = "black", fill = "white", pch = 21)

#Scaling
pxyPH = pxyPH + scale_x_continuous(limits = c(-4.75e5, -4.25e5))  #cont scale on the x axis 
pxyPH = pxyPH + scale_y_continuous(limits = c(-4e4,  4e4))   #cont scale on the y axis 
...
# Theme
pxyPH = pxyPH+legend_inside_theme+theme(legend.position = c(0.3,0))
pxyPH

# Add Li
dfemli       = dflibpoint(Li, FWRK)
pxyPH = pxyPH + geom_point(data = dfemli, aes(x= x_PH, y = y_PH), size = 4, colour = "black", fill = "black", pch = 21) 
pxyPH = pxyPH + annotate("text", x = dfemli$x_PH, y = -5e3,  label = "\\textsc{eml}$_2$", size=10)
...
#Scaling
pyzPH = pyzPH + scale_x_continuous(limits = c(-5e4, 5e4))  #cont scale on the x axis 
pyzPH = pyzPH + scale_y_continuous(limits = c(-10000, 13000))  #cont scale on the y axis 
...
pyzPH = pyzPH + geom_segment(aes(x = orbit$yPH[ai], y = orbit$zPH[ai], xend = orbit$yPH[ai+1], yend = orbit$zPH[ai+1]), 
                         ...
                         arrow = arrow(length = unit(0.4, "cm"), type = "closed"))
# Theme
pyzPH = pyzPH+legend_inside_theme
...
#Scaling
pxy = pxy + scale_x_continuous(limits = c(-1.22, -1.12))  #cont scale on the x axis 
pxy = pxy + scale_y_continuous(limits = c(-0.11,  0.11))   #cont scale on the y axis 
...
pxy = pxy + annotate("text", x = dfemli$x_SYS, y = -0.015,  label = "\\textsc{eml}$_2$", size=10)
