...
clabels = c("0.0", "1$T$", "2$T$", "3$T$", "4$T$", "5$T$")
cbreaks = seq(0.0, 5*Period, 1*Period)
...
for(i in dfnumber)
{
  dflist[[i]] = read.table(paste0(currentfolder, dftype, dfindex[i],"_Size_", size, "_s5_", s5s, "_test.txt"), header = F)
...
pH = pH + scale_x_continuous(breaks = cbreaks, labels = clabels)   #cont scale on the x axis 
#Display the plot
pH = pH + legend_inside_theme+legend_pos(c(0,0))
...
#Cont scale on the x axis 
pI = pI + scale_x_continuous(breaks = cbreaks, labels = clabels)   #cont scale on the x axis 
#Logscale on the y axis
pI = pI + scale_y_log10(breaks = 10^(-8:0*2), limits=c(1e-12,1e-2))
#Theme
pI = pI+legend_inside_theme+legend_pos(c(1,0))
#Display
pI
...
#Cont scale on the x axis 
pO = pO + scale_x_continuous(breaks = cbreaks, labels = clabels)   #cont scale on the x axis 
#Logscale on the y axis
pO = pO + scale_y_log10(limits=c(1e-10,1), breaks = 10^(-8:0*2))
#Theme
...
#Scaling
pxyPH = pxyPH + scale_x_continuous(limits = c(-1.515e8, -1.502e8), labels = scientific_format(digits = 4))  #cont scale on the x axis 
pxyPH = pxyPH + scale_y_continuous(limits = c(-7e5,  7e5), labels = scientific_format())   #cont scale on the y axis 
#Theme
...
pxyPH = pxyPH+legend_inside_theme+legend_pos(c(1,0))
pxyPH
...
#Scaling
pyzPH = pyzPH + scale_x_continuous(limits = c(-7e5,  7e5), labels = scientific_format())  #cont scale on the x axis 
pyzPH = pyzPH + scale_y_continuous(limits = c(-6e5,  6e5), labels = scientific_format())  #cont scale on the y axis 
...
pyzPH = pyzPH + geom_segment(aes(x = orbit$yPH[ai], y = orbit$zPH[ai], xend = orbit$yPH[ai+1], yend = orbit$zPH[ai+1]), 
                         colour = muted("blue"), 
                         arrow = arrow(length = unit(0.4, "cm"), type = "closed"))
pyzPH = pyzPH+legend_inside_theme+legend_pos(c(0,0))
...
#Scaling
pxy = pxy + scale_x_continuous(limits = c(-1.0125, -1.004))  #cont scale on the x axis 
pxy = pxy + scale_y_continuous(limits = c(-0.00465,  0.00458))   #cont scale on the y axis 
...
# Theme
pxy = pxy+legend_inside_theme+theme(legend.position = c(1,0))

# Add Li
dfemli       = dflibpoint(Li, FWRK)
pxy = pxy + geom_point(data = dfemli, aes(x= x_SYS, y = y_SYS), size = 4, colour = "black", fill = "black", pch = 21) 
pxy = pxy + annotate("text", x = dfemli$x_SYS, y = +0.0008,  label = "\\textsc{sel}$_2$", size=10)
...
#Scaling
pyz = pyz + scale_x_continuous(limits = c(-0.0048,  0.0048)) #cont scale on the x axis 
pyz = pyz + scale_y_continuous(limits = c(-0.0034, 0.0036))  #cont scale on the y axis 
...
pyz = pyz + geom_segment(aes(x = orbit$yEM[ai], y = orbit$zEM[ai], xend = orbit$yEM[ai+1], yend = orbit$zEM[ai+1]), 
                         ...
                         arrow = arrow(length = unit(0.4, "cm"), type = "closed"))
# Theme
pyz = pyz+legend_inside_theme+legend_pos(c(0,0))


# Add Li
dfemli       = dflibpoint(Li, FWRK)
pyz = pyz + geom_point(data = dfemli, aes(x= y_SYS, y = z_SYS), size = 4, colour = "black", fill = "black", pch = 21) 
pyz = pyz + annotate("text", x = dfemli$y_SYS, y = -0.0005,  label = "\\textsc{sel}$_2$", size=10)
...
#Save in tex file
#-------------------------------------------------------------------------------
ggplot2tikz_phd(pxy, xSize, ySize, file = paste0(currentfolder, "R_XY_Size_", sizep, "_s5_", s5c,  "_EM.tex"))
ggplot2tikz_phd(pyz, xSize, ySize, file = paste0(currentfolder, "R_YZ_Size_", sizep, "_s5_", s5c,  "_EM.tex"))
...
#Save in tex file
#-------------------------------------------------------------------------------
ggplot2tikz_phd(pI,  xSize, ySize, file = paste0(currentfolder, "R_eI_Size_", sizep, "_s5_", s5c, ".tex"))
ggplot2tikz_phd(pO,  xSize, ySize, file = paste0(currentfolder, "R_eO_Size_", sizep, "_s5_", s5c, ".tex"))
ggplot2tikz_phd(pH,  xSize, ySize, file = paste0(currentfolder, "R_eH_Size_", sizep, "_s5_", s5c, ".tex"))
ggplot2tikz_phd(pxyPH, xSize, ySize, file = paste0(currentfolder, "R_XY_Size_", sizep, "_s5_", s5c, ".tex"))
ggplot2tikz_phd(pyzPH, xSize, ySize, file = paste0(currentfolder, "R_YZ_Size_", sizep, "_s5_", s5c, ".tex"))
