...
size =  "0.6_test" #40_test
sizep = "06_test"# "1.5_test" #40_test
...
clabels = c("0.0", "2$T$", "4$T$", "6$T$", "8$T$", "10$T$")
cbreaks = seq(0.0, 10*Period, 2*Period)

# clabels = c("0.0", "$T$", "2$T$", "3$T$", "4$T$", "5$T$", "6$T$", "7$T$", "8$T$", "9$T$", "10$T$")
# cbreaks = seq(0.0, 10*Period, Period)
...
#Cont scale on the x axis 
pI = pI + scale_x_continuous(breaks = cbreaks, labels = clabels) 
#Logscale on the y axis
pI = pI + scale_y_log10(breaks = 10^(-8:0*2), limits=c(1e-12,1e-3))
#Theme
...
#Scaling
pxyPH = pxyPH + scale_x_continuous(limits = c(-1.484e8, -1.4785e8), labels = scientific_format())  #cont scale on the x axis 
pxyPH = pxyPH + scale_y_continuous(limits = c(-1e6,  1e6), labels = scientific_format())   #cont scale on the y axis 
#Theme
...
pxyPH = pxyPH+legend_inside_theme+theme(legend.position = c(1,0))
pxyPH
...
pxyPH = pxyPH + annotate("text", x = dfemli$x_PH, y = -1.3e5,  label = "\\textsc{sel}$_1$", size=10)
...
#Scaling
pyzPH = pyzPH + scale_x_continuous(limits = c(-1e6,  1e6), labels = scientific_format())  #cont scale on the x axis 
pyzPH = pyzPH + scale_y_continuous(limits = c(-5e5,  5e5), labels = scientific_format())  #cont scale on the y axis 
...
pyzPH = pyzPH + annotate("text", x = dfemli$y_PH, y = -0.6e5,  label = "\\textsc{sel}$_1$", size=10)
...
#Scaling
pxy = pxy + scale_x_continuous(limits = c(-0.9922, -0.988))  #cont scale on the x axis 
pxy = pxy + scale_y_continuous(limits = c(-0.006,  0.0055))   #cont scale on the y axis 
...
pxy = pxy + annotate("text", x = dfemli$x_SYS, y = -0.001,  label = "\\textsc{sel}$_1$", size=10)
...
#Scaling
pyz = pyz + scale_x_continuous(limits = c(-0.006,  0.0054)) #cont scale on the x axis 
pyz = pyz + scale_y_continuous(limits = c(-0.0035, 0.003))  #cont scale on the y axis 
...
pyz = pyz + annotate("text", x = dfemli$y_SYS, y = -0.0005,  label = "\\textsc{sel}$_1$", size=10)
