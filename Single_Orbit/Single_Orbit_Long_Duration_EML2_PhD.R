...
s3c = c(36)
s4c = c(1.74347)
filename = "orbit_qhalo_long_duration_small"
...
s3c = c(36)
s4c = c(2.81423)
filename = "orbit_qhalo_long_duration_medium"
...
s3c = c(-32.4545)
s4c = c(-3.48331)
filename = "orbit_qhalo_long_duration_thin"
...
# s3c = c(13.9091)
# s4c = c(1.55436)
# filename = "orbit_qhalo_long_duration_big"
...
pxy = pxy + scale_x_continuous(limits = c(-1.2, NaN))
pxy = pxy + scale_y_continuous(limits = c(-0.1, 0.1))

#Add m2 with true apparent radius
primaryPos  =  -(1-muR)
pxy = addPrimary(pxy, primaryPos, 0, primaryR/L, 0.4, "grey50")

#Add the name of the Moon
pxy = pxy + annotate("text", x = primaryPos-0.007, y = 0.015,  label = "Moon", size=7)
pxy
...
pxz = pxz + scale_x_continuous(limits = c(-1.2, NaN))
pxz = pxz + scale_y_continuous(limits = c(-0.12, 0.12))

#Add m2 with true apparent radius
primaryPos  =  -(1-muR)
pxz = addPrimary(pxz, primaryPos, 0, primaryR/L, 0.4, "grey50")

#Add the name of the Moon
pxz = pxz + annotate("text", x = primaryPos-0.007, y = 0.015,  label = "Moon", size=7)
pxz
...
ps1s3 = plotdf_path(orbits_eml2,  "s1", "s3",  "$s_1$",  "$s_3$", lineSize = line.size.norm)
ps1s3

ps2s4 = plotdf_path(orbits_eml2,  "s2", "s4",  "$s_2$",  "$s_4$", lineSize = line.size.norm)
ps2s4

#===============================================================================
# PLOTS: only the energy
#===============================================================================
clabels = c("0.0", "10$T$", "20$T$", "30$T$", "40$T$", "50$T$")
cbreaks = seq(0.0, 50*Period, 10*Period)
