...
#Theme
pI = pI+legend_inside_theme+legend_pos(c(0,0))
#Display
pI
...
#Scaling
pxyPH = pxyPH + scale_x_continuous(limits = c(-4.9e5, -4.34e5))  #cont scale on the x axis 
pxyPH = pxyPH + scale_y_continuous(limits = c(-1.2e4,  6.5e4))   #cont scale on the y axis 
#Theme
...
pxyPH = pxyPH+legend_inside_theme+legend_pos(c(0,0))
pxyPH
...
#Scaling
pyzPH = pyzPH + scale_x_continuous(limits = c(-1.2e4,  7e4))  #cont scale on the x axis 
...
pyzPH = pyzPH + geom_segment(aes(x = orbit$yPH[ai], y = orbit$zPH[ai], xend = orbit$yPH[ai+1], yend = orbit$zPH[ai+1]), 
                         colour = muted("blue"), 
                         arrow = arrow(length = unit(0.4, "cm"), type = "closed"))
pyzPH = pyzPH+legend_inside_theme+legend_pos(c(1,0))
...
#Scaling
pxy = pxy + scale_x_continuous(limits = c(-1.285, -1.13))  #cont scale on the x axis 
pxy = pxy + scale_y_continuous(limits = c(-0.03,  0.175))   #cont scale on the y axis 
...
# Theme
pxy = pxy+legend_inside_theme+legend_pos(c(0,0))

# Add Li
dfemli       = dflibpoint(Li, FWRK)
pxy = pxy + geom_point(data = dfemli, aes(x= x_SYS, y = y_SYS), size = 4, colour = "black", fill = "black", pch = 21) 
pxy = pxy + annotate("text", x = dfemli$x_SYS, y = 0.06,  label = "\\textsc{eml}$_2$", size=10)
...
#Scaling
pyz = pyz + scale_x_continuous(limits = c(-0.03,  0.175)) #cont scale on the x axis 
...
pyz = pyz + geom_segment(aes(x = orbit$yEM[ai], y = orbit$zEM[ai], xend = orbit$yEM[ai+1], yend = orbit$zEM[ai+1]), 
                         ...
                         arrow = arrow(length = unit(0.4, "cm"), type = "closed"))
# Theme
pyz = pyz+legend_inside_theme+legend_pos(c(1,0))


# Add Li
dfemli       = dflibpoint(Li, FWRK)
pyz = pyz + geom_point(data = dfemli, aes(x= y_SYS, y = z_SYS), size = 4, colour = "black", fill = "black", pch = 21) 
pyz = pyz + annotate("text", x = dfemli$y_SYS, y = -0.015,  label = "\\textsc{eml}$_2$", size=10)
