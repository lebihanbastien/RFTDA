...
for (i in vorders)  #loop on the orders
{
  ...
  #fileprefix = paste0(currentfolder, "eOm_", Type, "_ofs_",ofs_order,"_order_", toString(i))
...
pE = plotdf_line(ttm    = ttm_all, 
                 ...
                 isColorFac = 1,
                 lineTypeCol = "isG30",
                 lineTypeLabel = "Order > 20")
pE = pE+scale_x_continuous(limits=c(0.0, 0.02))
pE = pE+scale_y_continuous(limits=c(-8, 1))
pE
...
#------------------------------------------------
pXY = plotdf_line(ttm_all, "xPH", "yPH", "xPH", "yPH", "order", "Order", 1)
...
ggsave(pE, file=paste0(currentfolder, "R_eOm_", "ofs_" , ofs_order,  "_", Type, "_Energy_EML1_QBCP.pdf"))
ggsave(pE, file=paste0(currentfolder, "R_eOm_", "ofs_" , ofs_order,  "_", Type, "_Energy_EML1_QBCP.eps"))