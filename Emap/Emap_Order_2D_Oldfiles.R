...
for (i in vorders)  #loop on the orders
{
  # Filename to check
  #------------------------------------------------
  fileprefix = paste0(currentfolder, "eOm/eOm_", Type, "_ofs_",ofs_order,"_order_", toString(i))
...
for(i in 1:length(vorders))
{
  ...
  order = vorders[i];
  ttm_l[[i]]  = ttm_all[which(ttm_all$order == order),]      #select order
  ppl[[i]]    = plotdf_point(ttm_l[[i]], xi, yi, "x [km]", "z [km]", "log10eOm", "log10eOm", 0)
  ppl[[i]]    = ppl[[i]]+scale_colour_gradient2(legTitle, space="Lab", midpoint = -6, mid = "white", high = muted("blue"), guide = FALSE)
  
  #Earth direction
  ppl[[i]]    = ppl[[i]]+ annotate("text", x = -sign(moonFactor)*4000, y = -28000, label = "to Earth \ ", size = 6)
  ...
  #Add the moon
  moonR    = 1737.10/abs(moonFactor)  #Moon's radius
  moonPos  = gamma*L/moonFactor       #Moon position wrt to Li
  ppl[[i]] = addMoon(ppl[[i]], x = moonPos, y = 0, moonR, surfSize = 0.4, cratSize = 0.2)+ coord_fixed()
  ppl[[i]]    = ppl[[i]]+ annotate("text", x = sign(moonFactor)*64000, y = 5000, label = "Moon", size = 6)
}
#Actual plot in multiplot format
pMult_ph = multiplot(plotlist = ppl, cols = 2)

#Save to eps file
ggsave(ppl[[1]], file=paste0(currentfolder, "R_eOm_", "ofs_" , ofs_order, "_", vorders[1], "_", Type, "_withMoon.eps"))
