...
# Multiple Families
#===============================================================================
isMULTFAM  = FALSE #if TRUE, we suppose that we want to display multiple families (e.g. the iconic case of t0 = 0.99T)
NFAM       = 5
FAMVEC     = seq(1,NFAM,1)
...
if(isMULTFAM)
{
  ...
  #-----------------------------------------------------------------------------
  proj_cont_fam = get_proj_cont(FILE_PREFIX_CONT, FILE_SUFFIX_CONT, "_fam1")
  proj_cont_fam$family = 1
  
  #-----------------------------------------------------------------------------
  # Continuation results - trajectories
  #-----------------------------------------------------------------------------
  traj_cont_fam = get_traj_cont(FILE_PREFIX_CONT_TRAJ, FILE_SUFFIX_CONT_TRAJ, "_fam1", PRINT)
  traj_cont_fam$family = 1
  ...
  #-----------------------------------------------------------------------------
  #   label_min = lab_min_vec[1]
  #   label_max = max(proj_cont_label$label) - lab_max_vec[1]
  #   
  #   minmaxlabel = traj_cont_fam$label > label_min & traj_cont_fam$label < label_max
  #   traj_cont_fam = traj_cont_fam[which(minmaxlabel),]
  #   
  #   minmaxlabel = proj_cont_fam$label > label_min & proj_cont_fam$label < label_max
  #   proj_cont_fam = proj_cont_fam[which(minmaxlabel),]
  
  
  #-----------------------------------------------------------------------------
  # Same on all the families
  #-----------------------------------------------------------------------------
  for(FAM in seq(2,NFAM,1))
  {
    #-----------------------------------------------------------------------------
    # Continuation results
    #-----------------------------------------------------------------------------
    ...
    # Continuation results - trajectories
    #---------------------------------------------------------------------------
    traj_cont_temp = get_traj_cont(FILE_PREFIX_CONT_TRAJ, 
                                   FILE_SUFFIX_CONT_TRAJ, paste0("_fam", FAM), PRINT) 
    traj_cont_temp$family = FAM
    
    #---------------------------------------------------------------------------
    # Postprocess
    #---------------------------------------------------------------------------
    #Label only
    condition       = traj_cont_temp$t_CMU_SEM == min(traj_cont_temp$t_CMU_SEM)
    proj_cont_label = traj_cont_temp[which(condition),]
    #Copy label into proj_cont
    proj_cont_temp$label = proj_cont_label$label
    ...
    #     label_max = max(proj_cont_label$label) - lab_max_vec[1]
    #     
    #     minmaxlabel = traj_cont_temp$label > label_min & traj_cont_temp$label < label_max
    #     traj_cont_temp = traj_cont_temp[which(minmaxlabel),]
    #     
    ...
    traj_cont_fam = rbind_cc(traj_cont_fam, traj_cont_temp)
  }
