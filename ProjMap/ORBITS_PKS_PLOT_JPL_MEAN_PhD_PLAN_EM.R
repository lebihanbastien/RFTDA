...
# Load from file
#-------------------------------------------------------------------------------
load(file = "Rda/traj_from_jpl_mean_phd_plan.Rda")
...
# Plot
aes.pk = aes_string("dHf_SEM", "CST_GAMMA_LIB_EM*mean_n_NCEM",  group = cs_interact("label", re.string), color = scolor)
...
pp_mean_dHf_n_EM = pp_mean_dHf_n_EM + scale.color.noguide
...
if(is.saved)
{
  filename = paste0(FILE_PREFIX_FROM_JPL, "_pp_mean_nEM_nSEM")
...
if(is.saved)
{
  filename = paste0(FILE_PREFIX_FROM_JPL, "_pp_mean_nEM_dnEM")
...
if(is.saved)
{
  filename = paste0(FILE_PREFIX_FROM_JPL, "_pp_mean_nSEM_dnSEM")
...
if(is.saved)
{
  filename = paste0(FILE_PREFIX_FROM_JPL, "_pp_mean_nSEM_dnEM")
...
if(is.saved)
{
  filename = paste0(FILE_PREFIX_FROM_JPL, "_pp_mean_nEM_dnSEM")
...
aes.pk = aes_string("re_CMU_EMT_mod", "CST_GAMMA_LIB_EM*mean_dn_NCEM",  
                    group = cs_interact("label", re.string), color = "factor(seed)")
...
if(is.saved)
{
  filename = paste0(FILE_PREFIX_FROM_JPL, "_pp_mean_re_dnEM")
...
if(is.saved)
{
  filename = paste0(FILE_PREFIX_FROM_JPL, "_pp_mean_re_dnSEM")
...
aes.pk = aes_string("re_CMU_EMT_mod", "CST_GAMMA_LIB_SEM*mean_dn_NCSEM",  
                    group = cs_interact("label", re.string), 
                    color = "factor(seed)")
pp_mean_re_dn_SEM = ggplot() + geom_point(data = traj_from_jpl_mean, aes.pk, size = 3) 

#Color and shape
pp_mean_re_dn_SEM = pp_mean_re_dn_SEM + scale.color.jpl.local

# Theme
pp_mean_re_dn_SEM = pp_mean_re_dn_SEM + custom_theme

# Right font
pp_mean_re_dn_SEM = set_font_cm_x(pp_mean_re_dn_SEM)
...
aes.pk = aes_string("re_CMU_EMT_mod", "CST_GAMMA_LIB_EM*mean_dn_NCEM",  
                    group = cs_interact("label", re.string), 
                    color = "factor(seed)")
pp_mean_re_dn_EM = ggplot() + geom_point(data = traj_from_jpl_mean, aes.pk, size = 3) 

#Color and shape
pp_mean_re_dn_EM = pp_mean_re_dn_EM + scale.color.jpl.local

# Theme
pp_mean_re_dn_EM = pp_mean_re_dn_EM + custom_theme

# Right font
pp_mean_re_dn_EM = set_font_cm_x(pp_mean_re_dn_EM)
...
#-------------------------------------------------------------------------------
aes.pk = aes_string("CST_GAMMA_LIB_EM*mean_dn_NCEM", "CST_GAMMA_LIB_SEM*mean_dn_NCSEM",  
                    group = cs_interact("label", re.string), 
                    color = "factor(seed)")
pp_mean_dn_EM_dn_SEM = ggplot() + geom_point(data = traj_from_jpl_mean, aes.pk, size = 3) 

#Color and shape
pp_mean_dn_EM_dn_SEM = pp_mean_dn_EM_dn_SEM + scale.color.jpl.local

# Theme
pp_mean_dn_EM_dn_SEM = pp_mean_dn_EM_dn_SEM + custom_theme


pp_mean_dn_EM_dn_SEM
