...
#source("ProjMap/ORBITS_PKS_PLOT_JPL_MEAN_LOAD") #compute traj_from_jpl_mean (long! comment once done once) 

#-------------------------------------------------------------------------------
# Aes
#-------------------------------------------------------------------------------
scale.color.title.local = expression("Initial seed")
...
#-------------------------------------------------------------------------------
traj_from_jpl_mean_0 = traj_from_jpl_mean
traj_from_jpl_mean   = traj_from_jpl_mean_0[which(traj_from_jpl_mean_0$dHf_SEM < 1.4e-4),]
...
aes.pk = aes_string("CST_GAMMA_LIB_EM*mean_n_NCEM", "CST_GAMMA_LIB_SEM*mean_n_NCSEM",  
                    group = cs_interact("label", re.string), color = scolor)
...
#-------------------------------------------------------------------------------
# Plot r0_CMU_EMT_mod vs mean_dn_NCEM
#-------------------------------------------------------------------------------
pp_mean_re_nNCEM = ggplot() + geom_point(data = traj_from_jpl_mean, aes(x = r0_CMU_EMT_mod, y = mean_dn_NCEM)) 
# Labels on plot
pp_mean_re_nNCEM = pp_mean_re_nNCEM + geom_text(data = traj_from_jpl_mean, aes(x = r0_CMU_EMT_mod, y = mean_dn_NCEM, label=label),hjust=0, vjust=0)
# Theme
pp_mean_re_nNCEM = pp_mean_re_nNCEM + custom_theme
pp_mean_re_nNCEM


#-------------------------------------------------------------------------------
# Plot r0_CMU_EMT_mod vs mean_dn_NCSEM
#-------------------------------------------------------------------------------
pp_mean_re_nNCSEM = ggplot() + geom_point(data = traj_from_jpl_mean, aes(x = r0_CMU_EMT_mod, y = mean_dn_NCSEM)) 
# Labels on plot
pp_mean_re_nNCSEM = pp_mean_re_nNCSEM + geom_text(data = traj_from_jpl_mean, aes(x = r0_CMU_EMT_mod, y = mean_dn_NCSEM, label=label),hjust=0, vjust=0)
# Theme
pp_mean_re_nNCSEM = pp_mean_re_nNCSEM + custom_theme
pp_mean_re_nNCSEM
