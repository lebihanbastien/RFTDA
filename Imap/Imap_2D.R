...
# Plot of the orders given in the array vorders.
#
# 01/2016
############################################################
...
for(i in 1:length(vorders))
{
  ...
  ppl[[i]]    = plotdf_tile_n(ttm_l[[i]], si, sj, sis, sjs, "log10eOm", legTitle, -8, isLegendOn)
}
...
for(i in 1:length(vorders))
{
  ...
  ppl[[i]]    = ppl[[i]]+scale_colour_gradient2(legTitle, space="Lab", midpoint = -8, mid = "white", high = muted("blue"), guide = FALSE)
...
#   ppl[[i]] = ppl[[i]] + geom_point(data = ttm_s1s3,  aes(x = xPH, y = yPH), color = "brown", shape = 16, size = 2)
#   ppl[[i]] = ppl[[i]] + geom_point(data = ttm_s1ms3,  aes(x = xPH, y = yPH), color = "brown", shape = 16, size = 2)
...
#Save to eps file
ggsave(ppl[[1]], file=paste0(currentfolder, "R_eIm_", "ofs_" , ofs_order, "_", vorders[1], "_", Type, "_dH_", xi, yi,".eps"))
