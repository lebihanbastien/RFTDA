#-------------------------------------------------------------------------------
# fpp_path_traj function to plot trajectories from refinement/continuation 
# procedures
#
                          ...
                          isPNG = TRUE, isPDF = TRUE, isPrimary = TRUE)
{
  ...
  if(fwrk == "SEM")
  {
    ...
    fplot.pdf = fplot + annotate("text", x = 0, y = y.position.labels,  
                                 label = paste0("bold(SEML[", num.sem, "])"), colour = "white",
                                 size = 5, parse = TRUE)
    ...
    #---------------------------------------------------------------------------
    # SEMLi
    fplot.tex = fplot + annotate("text", x = 0, y = y.position.labels,  label = paste0("\\textsc{seml}$_", num.sem, "$"), size = size.txt)
    if(complementary.objects)
    {
  ...
  else if(fwrk == "EM")
  {
    ...
    #---------------------------------------------------------------------------
    # EMLi
    fplot.pdf = fplot + annotate("text", x = 0, y = -0.08,  
